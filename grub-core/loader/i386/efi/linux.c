...
grub_linuxefi_unload (void)
{
  ...
  kernel_free(initrd_mem, params->hdr.ramdisk_size);
  kernel_free(linux_cmdline, params->hdr.cmdline_size + 1);
                 ...
                 int argc, char *argv[])
{
  ...
  params->hdr.ramdisk_size = LOW_U32(size);
  params->hdr.ramdisk_image = LOW_U32(initrd_mem);
		...
		int argc, char *argv[])
{
  ...
  grub_dprintf ("linux",
		...
		params->hdr.ext_loader_type, params->hdr.ext_loader_ver);
